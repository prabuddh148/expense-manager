...
`render.yaml` in the repository root describes the whole service, so Render can create it
without any manual field entry:

1. Create the database first (above) and keep the JDBC URL, user and password to hand.
2. Render dashboard -> **New** -> **Blueprint** -> select this repository.
3. Render reads `render.yaml`, then prompts only for the values marked `sync: false`:
   `DB_URL`, `DB_USERNAME`, `DB_PASSWORD` and `GOOGLE_CLIENT_IDS` (blank is fine).
   `JWT_SECRET` is generated by Render and never leaves it.
4. Apply. The first build takes a few minutes because it compiles inside Docker.

The service is live at `https://<service-name>.onrender.com`; check
`https://<service-name>.onrender.com/api/health` returns `{"status":"UP"}`.

`backend/Dockerfile` is a two-stage build - Temurin 25 JDK to compile, JRE to run - with a
non-root user and `MaxRAMPercentage` set so the JVM sizes itself to the free tier's memory
limit rather than the host's.

Hosts other than Render work the same way: point them at `backend/Dockerfile` and supply the
...
EAS builds in the cloud and prints a download link when it finishes; open that link on the
phone to install. The URL is compiled into the bundle, so changing the backend address means
rebuilding.

Note that a free Render instance sleeps after inactivity. The first request from a cold app
can take ~50 seconds, during which the app shows its retry state rather than an error.
